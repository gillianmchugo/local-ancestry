# inbreeding filter plot
## plot inbreeding results after filtering
...
plot <- readr::read_table("03_filter-data/04_inbreeding/02_filter/ars-ld-mind-ibs-het-filter.het") %>%
  dplyr::group_by(FID) %>%
  dplyr::mutate(group = dplyr::case_when(FID %in% c("HOLS", "ANGU", "JERS") ~ "European Bos taurus",
...
### save plot
png("03_filter-data/04_inbreeding/02_filter/inbreeding-filter.png",
    height = 6,
    res = 300,
    units = "in",
    width = 9)
print(plot)
dev.off()
pdf("03_filter-data/04_inbreeding/02_filter/inbreeding-filter.pdf",
    height = 6,
    width = 9)
print(plot)
dev.off()